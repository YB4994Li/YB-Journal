...
const Auto = () => <span className="ml-1 rounded bg-sky-500/10 px-1.5 text-[10px] uppercase text-sky-300">Auto</span>;
export default function TradeViewModal({ trade, currency, onClose, onImage, onDeleteScreenshot }) {
  ...
  return <Modal open onClose={onClose} title={`Trade #${trade.tradeNumber}`} subtitle={`${trade.market} · ${trade.direction}`} wide>
    ...
    {trade.calculationWarnings?.length ? <div className="mt-5 rounded-xl border border-amber-500/30 bg-amber-500/5 p-4 text-sm text-amber-200" title={trade.calculationWarnings.join('\n')}>{trade.calculationWarnings.join('; ')}</div> : null}
