...
function DayTradeSnapshot({trade,currency,centered}){
  ...
  return <article className={`min-w-[390px] flex-1 rounded-xl border bg-ink/35 p-4 text-left ${resultBorder[trade.result]||resultBorder.BREAK_EVEN} ${centered?'max-w-[560px]':''}`}>
...
export default function DayTradeDrawer({day,trades,currency,onClose}){
  ...
  return createPortal(<div className="fixed inset-0 z-[9999] flex h-[100dvh] w-screen items-center justify-center bg-black/70 p-3 backdrop-blur-[5px] sm:p-6" onMouseDown={(event)=>event.target===event.currentTarget&&onClose()}>
    <section role="dialog" aria-modal="true" aria-label={`Daily trading snapshot for ${date}`} className="day-snapshot-in card flex max-h-[min(86dvh,680px)] w-[min(1180px,92vw)] flex-col overflow-hidden shadow-[0_30px_100px_rgba(0,0,0,.65)]">
      ...
      <div className="performance-scrollbar overflow-x-auto overflow-y-auto p-4 sm:p-5"><div className={`flex gap-3 ${trades.length===1?'justify-center':''}`}>{trades.map((trade)=><DayTradeSnapshot key={trade.id} trade={trade} currency={currency} centered={trades.length===1}/>)}</div></div>
    </section>
  </div>,document.body);
}
