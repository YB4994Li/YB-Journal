...
function RadarTooltip({active,payload,chosen}){if(!active||!payload?.length)return null;const key=String(payload[0].dataKey),category=chosen.find((row)=>String(row.key)===key),color=seriesColor(category?.name);return <PerformanceChartTooltip active={active} payload={payload} title={payload[0].payload.metric} color={color} valueFormatter={(value,name)=>`${chosen.find((row)=>String(row.key)===String(name))?.name||name}: ${number(value,1)}/100`}/>}
