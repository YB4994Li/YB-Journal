...
export default function StrategyRadarChart({rows,currency}){
  ...
  const data=radarSeries(chosen),colorFor=(row)=>seriesColor(eligible.findIndex((item)=>String(item.key)===String(row.key)));
  ...
  return <section className="border-t border-line pt-4">
    <div className="flex flex-wrap gap-2">{eligible.map((row)=><button key={row.key} className={selected.includes(String(row.key))?'btn-primary py-1.5':'btn-secondary py-1.5'} onClick={()=>toggle(String(row.key))}>{row.name}</button>)}</div>
