...
export default function BreakdownDashboard({rows,tab,metric,setMetric,currency,onSelect}){
  ...
  const comparison=tab==='markets'?<MarketProfitFactorLollipopChart rows={ordered} currency={currency} onSelect={onSelect}/>:tab==='sessions'||tab==='directions'?<DashboardPanel title={`${label} comparison`} subtitle="Gross profit versus absolute gross loss"><ResponsiveComparison rows={ordered} onSelect={onSelect}/></DashboardPanel>:<PerformanceLineChart title={`Profit Factor by ${label}`} rows={ordered} metric="profitFactor" onSelect={onSelect}/>;
