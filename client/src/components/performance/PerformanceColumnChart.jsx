...
export default function PerformanceColumnChart({title,subtitle,rows,metric='netProfitLoss',currency,onSelect,large=false,maxBarSize,zeroBaseline=false}){
  ...
  const domain=metric==='winRate'?[0,100]:large&&metric==='netProfitLoss'?directionPnlDomain:zeroBaseline?[0,'auto']:['auto','auto'];
  return <DashboardPanel title={title} subtitle={subtitle}>
    {!data.length?<div className="flex h-56 items-center justify-center text-sm text-muted">No data to visualize.</div>:<div className="performance-scrollbar max-w-full overflow-x-auto"><div style={{minWidth:Math.max(420,data.length*76)}}>
      <ResponsiveContainer width="100%" height={large?330:260}>
        <BarChart layout="horizontal" data={data} margin={{top:22,right:8,left:0,bottom:18}}>
          ...
          <Tooltip content={<PerformanceChartTooltip valueFormatter={(value)=>format(value,metric,currency)}/>} cursor={{fill:'rgba(255,255,255,.025)'}}/>
          {metric==='netProfitLoss'&&<ReferenceLine y={0} stroke="#94a3b8" strokeOpacity={.65}/>} 
          <Bar dataKey="value" maxBarSize={maxBarSize??(large?52:58)} radius={[7,7,2,2]} strokeWidth={1} isAnimationActive animationDuration={480} animationEasing="ease-out" onClick={(entry)=>onSelect?.(entry.payload||entry)} cursor={onSelect?'pointer':'default'}>
