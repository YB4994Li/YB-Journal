...
export default function FundedChallengeSetup({ mode,setMode,phases,setPhases,accountSize }) {
  ...
  const add=()=>setPhases((current)=>[...current,phase(`Custom phase ${current.filter((item)=>item.phaseType!=='FUNDED_LIVE').length+1}`,'CUSTOM',accountSize,current.some((item)=>item.status==='ACTIVE')?'PENDING':'ACTIVE')]);
  const hasLive=phases.some((item)=>item.phaseType==='FUNDED_LIVE');
  const toggleLive=()=>setPhases((current)=>hasLive?current.filter((item)=>item.phaseType!=='FUNDED_LIVE'):[...current,phase('Funded / Live','FUNDED_LIVE',accountSize,current.some((item)=>item.status==='ACTIVE')?'PENDING':'ACTIVE')]);
