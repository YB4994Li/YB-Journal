...
import { scopedModuleUrl } from '../../utils/activeAccount.js';
...
function NavigationItem({ item, collapsed, onNavigate, accountId, phaseId }) {
  const Icon = item.icon;
  return <NavLink to={['/journal','/performance','/calendar','/daily-notes'].includes(item.to) ? scopedModuleUrl(item.to, accountId, phaseId) : item.to} onClick={onNavigate}
...
export default function Sidebar({ collapsed, mobileOpen, onToggle, onClose }) {
  const { activeAccountId, activePhaseId } = useActiveAccount();
  return <>
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 flex w-72 flex-col border-r border-line bg-[#090d13] transition-transform duration-200 ${mobileOpen ? 'translate-x-0' : '-translate-x-full'} md:sticky md:top-0 md:h-screen md:w-full md:translate-x-0`} aria-label="Application navigation">
      ...
      <nav className="flex flex-1 flex-col gap-1 overflow-y-auto p-3" aria-label="Primary navigation">{primaryItems.map((item) => <NavigationItem key={item.to} item={item} collapsed={collapsed} onNavigate={onClose} accountId={activeAccountId} phaseId={activePhaseId}/>)}<div className="mt-auto border-t border-line pt-3"><NavigationItem item={{ label: 'Settings', to: '/settings', icon: Settings }} collapsed={collapsed} onNavigate={onClose}/></div></nav>
