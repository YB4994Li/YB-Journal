...
export default function Modal({ open, onClose, title, subtitle, children, wide = false }) {
  ...
  return <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/75 p-4 backdrop-blur-sm" onMouseDown={(e) => e.target === e.currentTarget && onClose()}>
    <div className={`card max-h-[92vh] w-full overflow-hidden ${wide ? 'max-w-5xl' : 'max-w-2xl'}`}>
      <div className="flex items-start justify-between border-b border-line px-6 py-5">
        ...
        <button className="rounded-lg p-2 text-muted hover:bg-white/5 hover:text-white" onClick={onClose}><X size={18}/></button>
      </div>
